...
class MixedOp(nn.Module):
    ...
    def forward(self, x, weights):
        Opx = [op(x) for op in self._ops]
        mix_o = sum(w * opx for w, opx in zip(weights, Opx))
        return mix_o


class SearchCell(nn.Module):

    def __init__(self, C, stride, num_nodes=4, keep_mask=None):
        super(SearchCell, self).__init__()
        self.num_nodes = num_nodes
        self.options = nn.ModuleList()
        for curr_node in range(self.num_nodes-1):
            for prev_node in range(curr_node+1): 
                self.options.append(MixedOp(C, stride))
    ...
    def forward(self, x, weights):
        ...
        for curr_node in range(self.num_nodes-1):
            ...
            for prev_node in range(curr_node+1): # n-1 prev nodes
                output = self.options[idx](outs[prev_node], weights[idx])
                edges_in.append(output)
                idx += 1
...
OPS = {
    'none' : lambda C, stride, affine: Zero(stride, name='none'),
    'avg_pool_3x3' : lambda C, stride, affine: POOLING(3, 1, 1, name='avg_pool_3x3'),
    'nor_conv_3x3' : lambda C, stride, affine: ReLUConvBN(C, C, 3, 1, 1, 1, affine, name='nor_conv_3x3'),
    'nor_conv_1x1' : lambda C, stride, affine: ReLUConvBN(C, C, 1, 1, 0, 1, affine, name='nor_conv_1x1'),
    'skip_connect' : lambda C, stride, affine: Identity(name='skip_connect'),
}
